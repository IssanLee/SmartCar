...
typedef struct 
{
  ...
  */
  ...
  /*
    描述：选择DMA通道对应的请求源
    取值：见本页"DMA request sources Number"宏定义
    初始化：必须初始化
  */
  uint8  DMA_Req; 
  ...
  */  
  ...
  /*
    ...
    初始化：不必须初始化，默认DMA_SRC_8BIT
  */
  uint8 DMA_SourceDataSize;
  
  /*
    描述：配置源数据地址偏移，即在上次读取后源地址的偏移量
    取值：16位有符号整型（-32768~32767）
    初始化：不必须初始化，默认0
  */ 
  int16 DMA_SourceAddrOffset;
  
  /*
    描述：配置源数据地址在主循环完成后的调整量
    取值：32位有符号整型
    初始化：不必须初始化，默认0
  */ 
  ...
  */  
  ...
  /*
    ...
    初始化：不必须初始化，默认DMA_DST_8BIT
  */
  uint8 DMA_DestDataSize;
  
  /*
    描述：配置目的数据地址偏移，即在上次写入后目的地址的偏移量
    取值：16位有符号整型（-32768~32767）
    初始化：不必须初始化，默认0
  */ 
...
//DMA服务请求软件开始
void DMA_SoftwareStartService(DMA_InitTypeDef);
