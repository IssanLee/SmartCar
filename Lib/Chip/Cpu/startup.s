...
; * Summary:      兼容CMSIS规范的Freescale _MK60DZ10启动文件
; * Version:      V0.1
; * Author:       Leopold & LPLD
; * Date:         2019/1/27
; * Change Logs:
...
__vector_table
  ...
  ;;格式：DCD?expr?[,expr]?…??;Generates?32-bit?constants.生成32bit常数??
  DCD     sfe(CSTACK)               ; Top of Stack      ;;格式：SFE?(section?[{+?|?-}?offset])??返回section段结束地址，SFB(section)返回section段开始地址;??
...
;;
  ...
  PUBWEAK Reset_Handler                 ;;弱定义一个函数，当复位异常时跳转到Reset_Handler
  SECTION .noinit:CODE:REORDER:NOROOT(2);;兼容IAR7.2版本
  ;复位函数
  Reset_Handler
    LDR     R0, =SystemInit         ;执行系统初始化函数SystemInit(进行系统时钟的初始化)
    BLX     R0                      ;跳转到SystemInit
